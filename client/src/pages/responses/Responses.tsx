...
import { ArrowLeft, Download, Trash2, Star, BarChart2, MessageSquare, ChevronDown, ChevronUp, QrCode, X } from 'lucide-react';
    ...
    <div className="bg-canvas border border-hairline rounded-xl p-6 space-y-4">
      ...
      <div className="space-y-2">
        {shown.map((v: any, i: number) => (
          <div key={i} className="bg-surface-soft rounded-md px-3 py-2 text-sm text-ink font-sans border border-hairline">
            {v?.toString() || '—'}
          </div>
        ))}
...
export default function Responses() {
  ...
  if (isLoading) return <div className="p-12 text-center text-muted font-sans">Loading...</div>;
                  ...
                  {responses.map((response: any) => (
                    <tr key={response._id} className="bg-canvas border-b border-hairline hover:bg-surface-soft transition-colors">
                      ...
                      {form?.fields.map((f: any) => {
                        ...
                        const display = f.type === 'rating' && val
                          ? `${'★'.repeat(Number(val))}${'☆'.repeat(5 - Number(val))} (${val})`
                          : val?.toString() || '—';
                        return (
